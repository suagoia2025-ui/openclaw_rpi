...
- [ ] `WHISPER_BIN`, `WHISPER_MODEL`, `LLAMA_COMPLETION` (o `LLAMA_CLI` → `llama-completion`), `PHI3_GGUF`, `PIPER_BIN`, `PIPER_VOICE_ONNX`, `PIPER_VOICE_JSON` están definidos (p. ej. en `voice-pipeline/.env`).
- [ ] (Opcional) Mic en vivo: `arecord -l`, `VOICE_ALSA_CAPTURE` si hace falta, `./scripts/live_mic_pipeline.sh`.
